<script>
                ...
                success: function (data) {
                    ...
                    if (data.responseState !== 'ERROR' && data.responseState !== 'ERROR2') {
                       ...
                       /* var boxWithdrawAmount =  window.parent.$('.withdrawAmountFront:eq(1)',window.parent.document).val();
                        if(parseFloat(boxWithdrawAmount) !== parseFloat(data.response.resultedReceivingAmount)){
                            data.response.resultedReceivingAmount = boxWithdrawAmount;
                        }*/
                        swal({
                            ...
                            $.ajax({
                                ...
                                success: function (data2) {
                                    if(data2.responseState === 'ERROR4'){
                                        ...
                                        toastr.success('Withdraw of ' + data2.response.amount + ' ' + data2.response.currency_code + ' was sent.', 'Deposit History');
                                        swal({
                                            title: "Withdraw Request Succeed",
                                            text: 'Withdraw of ' + data2.response.amount + ' ' + data2.response.currency_code + ' was sent on: \n' + data2.response.withdrawal_address.address + '\nStatus: ' + data2.response.status,
                                            type: "success",
                                            showCancelButton: false,
                                            closeOnConfirm: true,
    ...
    function load_deposit_history(previewPage,pageLoad,pageResultsCount,offset){
        ...
        $.ajax({
            url: "ajP-Stex.php",
            type: "POST",
            data: {action: 'checkDepositHistory', transactionId:transactionId,giveCoin:giveCoin,apiKey:apiKey,apiSecret:apiSecret,pageResultsCount:pageResultsCount,offset:offset},
            dataType: "json",
            success: function (data) {
                if(data.responseState === 'ERROR'){
                    ...
                    var extraHtml = '' +
                        ...
                        '        <th data-hide="depositAmount" >Explorer</th>' +
                        '    </tr>' +
                        '    </thead><tbody>';
                    ...
                    for (var i = 0; i < data.response.length ; i++) {
                        ...
                        extraHtml+= papa+
                            ...
                            '<td><div><button type="button" class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-lg" data-url="'+data.response[i].explorerFullUrl+'">Explorer</button></div></td>' +
                            '</tr>';
                    }
                ...
                for (var i = 0; i < data.response.length-1 ; i++) {
                    ...
                    if(data.response[i].txid) {
                        extraHtml += '<tr>' +
                            ...
                            '<td><div><button type="button" class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-lg" data-url="'+data.response[i].explorerFullUrl+'">View On Explorer</button></div></td>' +
                            '</tr>';
                    }else{
                        extraHtml += '<tr>' +
                            '<td><img src="' + data.response[i].coinImage + '" style="width:20px;">' + data.response[i].amount + ' ' + data.response[i].currency_code + '</td>' +
    ...
    function placeOrder(priceType) {
        if(priceType === 'market'){
            swal({
                ...
                text: 'Stex does not provide market orders!',
                type: 'error',
                showCancelButton: false,
                confirmButtonColor: "#30966c",
                confirmButtonText: "OK",
                            ...
                            swal({
                                ...
                                var recall = function () {
                                    $.ajax({
                                        ...
                                        success: function (data2) {
                                            $.ajax({
                                                ...
                                                success: function (data26) {
                                                    var currentBalanceInSelectedGetCoin = data26.response.getCoin;
                                                    if (data2.response !== 'ERROR') {
                                                        var extraFills = '';
                                                        if (data2.boughtAmount > 0) {
                                                            var hasError = false;
                                                        ...
                                                        if (hasError) {
                                                                var data33 = data26;
                                                                    errorResponseMessage += "\nBalance: " + data33.response.giveCoin + " " + giveCoin;
                                                                    errorResponseMessage += "\nBalance: " + data33.response.getCoin + " " + getCoin;
                                                                    if(data2.errorMsg) {
                                                                        errorResponseMessage += "\n" + data2.errorMsg;
                                                                    }
                                                                    swal({
                                                            ...
                                                            if (data2.ordersCompleted.length) {
                                                                ...
                                                                for (var j = 0; j < data2.ordersCompleted.length; j++) {
                                                                    if (data2.ordersCompleted[j].status !== 'FAILED') {
                                                                        extraFills += '\n' + (j + 1) + ') Price: ' + data2.ordersCompleted[j].price + ' / Qty: ' + data2.ordersCompleted[j].amount;
                                                                    } else {
                                                                ...
                                                                extraFills += '\n------------\n';
                                                                extraFills += '\nBalance: '+currentBalanceInSelectedGetCoin + ' '+getCoin;
                                                            }
                                                        ...
                                                        swal({
                                                            title: priceType.toUpperCase() + " Order can't proceed",
                                                            text: 'You must detect deposit first in order to create order.'+extraMsg,
                                                            type: 'error',
                                                            showCancelButton: false,
                                                            confirmButtonColor: "#30966c",
                                                            confirmButtonText: "OK",
                                                ...
                                                complete:function(completeData){
                                                    setTimeout(function(){
                                                        writeDownLog('CHECK BALANCE OF COIN','<?=$_GET['transactionId']?>',JSON.stringify(completeData.responseJSON));
                                                    },2000);
                                                }
                                            });
                                        ...
                                        complete: function(completeData){
                                            setTimeout(function(){
                                                writeDownLog('BUY '+orderType,'<?=$_GET['transactionId']?>',JSON.stringify(completeData.responseJSON));
                                            },2000);
                                        }
                                    });
                                };
                            ...
                            swal({
                                ...
                                var recall = function () {
                                    $.ajax({
                                        ...
                                        complete: function(completeData){
                                            setTimeout(function(){
                                                writeDownLog('FIRST BUY NORMAL','<?=$_GET['transactionId']?>',JSON.stringify(completeData.responseJSON));
                                            },2000);
                                        }
                                    });
                                };
            ...
            $.ajax({
                url: "ajP-Stex.php",
                type: "POST",
                data: {action: 'checkOrderInfo', txId: transactionId},
                dataType: "json",
                success: function (data) {
                    if(data.response.withdrawExplTxId === null){
                        if(data.responseState === 'OK' && data.response.depositExplTxId !== ''){
