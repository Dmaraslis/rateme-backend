...
if(isset($_REQUEST['action'])) {
    ...
    if($_REQUEST['action'] == 'sendAllToMainAccount'){
        ...
        $swapReturn['response'] = $bequant->transfer_all_balance_from_sub_to_main('','');
    ...
    if($_REQUEST['action'] == 'checkBalances') {
        $balances = $bequant->gather_balances('removeNoBalance');
        $swapReturn['response'] = $balances;
        $meter = 0;
        foreach ($balances as $balance){
    ...
    if($_REQUEST['action'] == 'transferBalance'){
        $orderInfos = $order->swap_by_id($_REQUEST['txId']);
        $transfer = $bequant->transfer_balance_between_accounts($_REQUEST['type'], $orderInfos);
        $swapReturn['response'] = $transfer;
        if($transfer){
            $swapReturn['responseMsg'] = 'Convert Succeed';
        }else{
            $swapReturn['responseMsg'] = $bequant->error;
        }
    }

    if($_REQUEST['action'] == 'convertBalance'){
        $orderInfos = $order->swap_by_id($_REQUEST['txId']);
        $convert = $bequant->convert_coins($_REQUEST['type'], $orderInfos);
        $swapReturn['response'] = $convert;
        if($convert){
            $swapReturn['responseMsg'] = 'Convert Succeed';
        }else{
            $swapReturn['responseMsg'] = 'Convert Failed Please try again reason: '.$bequant->error;
        }
    }
    ...
    if($_REQUEST['action'] == 'placeOrder') {
        ...
        if($orderInfos['depositExplTxId']!= null) {
            /** AFTO EINAI TO FIX GIA TO USDT GIATI STO BEQUANT TO EXEI USD*/
            ...
            $serchedASC = $bequant->gather_symbol_infos($mixedSymbolASC);
            $serchedDESC = $bequant->gather_symbol_infos($mixedSymbolDESC);
            if($_REQUEST['orderType'] == 'firstNormal' || $_REQUEST['orderType'] == 'secondCross') {
                $bequantWithdrawFees = $bequant->gather_withdraw_fees($_REQUEST['getCoin'],$_REQUEST['withdrawAmount']);
                if(!$bequantWithdrawFees->error){ $withdrawalFee = floatval($bequantWithdrawFees); }else{ $withdrawalFee = 0; }
            }else{
                $withdrawalFee = 0;
            }
            if(!$serchedASC->error){//BUY
                $asksOrderBook = $bequant->gather_order_book($mixedSymbolASC,$_REQUEST['withdrawAmount']);
                /** AFTO EINAI TO FIX GIA TO USDT GIATI STO BEQUANT TO EXEI USD*/
                ...
                if($_REQUEST['giveCoin'] == $serchedASC->feeCurrency){
                    ...
                    $orderBookExtraInfos = $bequant->find_buy_price_and_weight_average('ask',$asksOrderBook,$net_deposit_amount);
                    ...
                    $move = $bequant->addTradingOrdersByPair($mixedSymbolASC, 'BUY','limit', $buyAmount, $orderBookExtraInfos['lastSelectedPrice'],$orderInfos['id'],'IOC');
            ...
            if(!$serchedDESC->error){//sell
                $bidsOrderbook = $bequant->gather_order_book($mixedSymbolDESC,$_REQUEST['depositAmount']);
                /** AFTO EINAI TO FIX GIA TO USDT GIATI STO BEQUANT TO EXEI USD*/
                ...
                if($_REQUEST['giveCoin'] == $serchedDESC->feeCurrency){
                    ...
                    $net_deposit_amount = $_REQUEST['depositAmount'] - $tradingCost - $IS_fee - $partnerFee;
                    $orderBookExtraInfos = $bequant->find_buy_price_and_weight_average('bid',$bidsOrderbook,$net_deposit_amount);
                    ...
                    $move = $bequant->addTradingOrdersByPair($mixedSymbolDESC, 'SELL','limit', $sellAmount, $orderBookExtraInfos['lastSelectedPrice'],$orderInfos['id'],'IOC');
                    if($move->code){ http_response_code(450); echo json_encode($move); exit(); }
                    if($move->status !== 'filled'){ http_response_code(450); $jsonReturn['repeat'] = 'Order is not filled and was canceled. Try again'; echo json_encode($jsonReturn); exit(); }
                     $boughtAmount = 0;
                     foreach ($move->tradesReport as $tradeReport =>$value){
    ...
    if($_REQUEST['action'] == 'checkDepositHistory') {
        $deposits = $bequant->gather_deposits(array('currency'=>$_REQUEST['giveCoin'], 'limit'=>$_REQUEST['pageResultsCount'], 'offset'=>$_REQUEST['offset']));
        $balances = $deposits;
        ...
        foreach ($balances as $balance => $value) {
            ...
            $checkIfExistsOutsideTxReason = $order->check_if_deposit_exists_on_god_table_by_transaction_hash_and_deposit_amount($balances[$balance]->hash,number_format($balances[$balance]->amount,$coinInfos['allowedDecimals'],'.',''));
        ...
        $swapReturn['response'] = $balances;
        $swapReturn['USED'] = $_SESSION['BQCredsUsedForThisTx'];
    }
    ...
    if($_REQUEST['action'] == 'withdrawAction') {
        ...
        $withdrawCurrBeq = $bequant->gather_withdraw_fees($transactionInfos['getCoin'], $transactionInfos['resultedReceivingAmount']);
        ...
        if ($transactionInfos['destinationTagPhrase'] == 'is not required') {
            $response = $bequant->addWithdrawal($transactionInfos['getCoin'], $withdrawAmount, $transactionInfos['clientRefundAddress'], $encryption_class->safeEncrypt(($transactionInfos['id'])));
        } else {
            $response = $bequant->addWithdrawal($transactionInfos['getCoin'], $withdrawAmount, $transactionInfos['clientRefundAddress'], $encryption_class->safeEncrypt(($transactionInfos['id'])), $transactionInfos['destinationTagPhrase']);
        }
        if ($response) {
            $swapReturn['responseState'] = 'OK';
            ...
            $swapReturn['responseState'] = 'ERROR4';
            if($bequant->error->description){
                $swapReturn['response'] = $bequant->error->description;
            }else{
                $swapReturn['response'] = $bequant->error->message;
            }
        }
    }
    ...
    if($_REQUEST['action'] == 'refundAction') {
        ...
        $withdrawCurrBeq = $bequant->gather_withdraw_fees($transactionInfos['givenCoin'], $transactionInfos['depositAmount']);
        if (!$withdrawCurrBeq->error) {
            $withdrawalFee = floatval($withdrawCurrBeq);
        } else {
            $swapReturn['responseState'] = 'ERROR';
        ...
        $bequant->move_balance_for_refund_if_needed($transactionInfos);
        sleep(0.2);
        if ($transactionInfos['destinationTagPhrase'] == 'is not required') {
            $response = $bequant->addWithdrawal($transactionInfos['givenCoin'], $finalRefund, $transactionInfos['clientReceiveAddress'], $encryption_class->safeEncrypt(($transactionInfos['id'])));
        } else {
            $response = $bequant->addWithdrawal($transactionInfos['givenCoin'], $finalRefund, $transactionInfos['clientReceiveAddress'], $encryption_class->safeEncrypt(($transactionInfos['id'])), $transactionInfos['destinationTagPhrase']);
        }
        if ($response) {
            $swapReturn['responseState'] = 'OK';
            $swapReturn['response'] = $response->id;
            ...
            $swapReturn['responseState'] = 'ERROR4';
            $swapReturn['test'] = $bequant->error->takis;
            if($bequant->error->description){
                $swapReturn['response'] = $bequant->error->description;
            }else{
                $swapReturn['response'] = $bequant->error->message;
